...
client = OpenAI(
    api_key="sk-JIZOIELDEQ2JE6MA5jRZ13a9LoIup2VGx5rt2x52AN3o5ZHr",
    base_url="https://api.chatanywhere.tech"
 # base_url="https://api.chatanywhere.org/v1"
)
# 定义文件路径
json_file_path = '/home/extra1T/jin/GoMate/RAG/retrieve/RAG/data/final_contexts.json'

# 读取 JSON 文件
with open(json_file_path, 'r', encoding='utf-8') as file:
    final_contexts = json.load(file)
    final_contexts = final_contexts
with open('/home/extra1T/jin/GoMate/RAG/retrieve/RAG/data/final_contexts_rewrite.json', 'r', encoding='utf-8') as f_json:
    context_data = json.load(f_json)
...
with open(output_file_path, 'w', encoding='utf-8', newline='') as csvfile:
    ...
    flag=0
    for idx, (context_item, question_itm) in tqdm(enumerate(zip(context_data, final_contexts)),desc="Processing questions", unit="个"):
        flag+=1
        question = question_itm['query']
        context = context_item['context']
        prompt = prompt_template.format(context=context, question=question)
        messages = [{'role': 'user','content': prompt}]
        answers = gpt_35_api(messages)
