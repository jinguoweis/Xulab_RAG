...
from assistance.client import run_client
import requests
...
from assistance.console.output_parser import BSOutputParser
from assistance.console.output_wrapper import display
...
class BSAgentExecutor:
    ...
    def run(self,
        ...
        # logger.setLevel(logging.INFO)  # 确保日志级别设置为 INFO
        idx = 0
        final_res = []
        ...
        route_name = run_client(self.config["qwen2_llm_url"], user_input)
        max_key,result = self.route_to_chain(route_name,user_input)
        print("最可能的key:", max_key)
        ...
        while True:
            ...
            if cx:
                ...
                llm_result = run_client(self.config["modelscope_url"],prompt111)
                if print_info:
            ...
            action, action_args = self.output_parser.parse_response(llm_result['answer'])
            if print_info:
