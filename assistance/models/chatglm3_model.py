...
class LlmModel:
    def __init__(self, model_path, config = {}):
        ...
        self.model = AutoModel.from_pretrained(model_path, trust_remote_code=True,device = 'cuda:9')
        # self.model = AutoModel.from_pretrained(model_path, trust_remote_code=True,device = 'cuda:9').half().quantize(8).cuda(9)
    ...
    def _chat(self, query: str, history: List[Tuple[str, str]] = None, max_length: int = 2048, num_beams=1,
        ...
        chat_result = self.model.chat(self.tokenizer, query, history, "user", max_length, num_beams, do_sample, top_p, temperature, logits_processor, **kwargs)
        return chat_result
...
if __name__ == "__main__":
    path = "/home/extra1T/model_embeding/chatglm3-6b"
    llm_model = LlmModel(path)
    print(llm_model.predict("你好呀"))